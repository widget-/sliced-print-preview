...
export const SEGMENT_VELOCITY_VERTEX_SHADER = /* glsl */ `
  precision highp float;
  ...
  uniform mat4 worldViewProjection;
  uniform mat4 view;
  uniform mat4 uPrevWorldViewProjection;
  ...
  void main() {
    ...
    float t = position.z + 0.5;
    vec3 segPos, endTangent;
    float layerZ, width;

    if (row2.z > 0.5) {
      vec3 p0 = row0.xyz; width = row0.w; vec3 p1 = row1.xyz;
      float w = row1.w; layerZ = row2.w;
      vec4 nextRow0 = texelFetch(segmentTexture, texelCoord(gl_InstanceID + 1, 0), 0);
      vec3 p2 = nextRow0.xyz;
      float mt = 1.0 - t;
      float denom = mt*mt + 2.0*t*mt*w + t*t;
      ...
      vec3 start = row0.xyz; layerZ = row0.w;
      vec3 end = row1.xyz; width = row1.w;
      segPos = mix(start, end, t);
      vec3 dir = end - start;
      float len = length(dir);
      endTangent = len > 0.001 ? dir / len : vec3(0.0, 0.0, 1.0);
    ...
    vClipPos = worldViewProjection * worldPos4;
    vPrevClipPos = uPrevWorldViewProjection * worldPos4;
...
export const VELOCITY_FRAGMENT_SHADER = /* glsl */ `
  ...
  void main() {
    vec2 curUV = vClipPos.xy / vClipPos.w;
    vec2 prevUV = vPrevClipPos.xy / vPrevClipPos.w;
